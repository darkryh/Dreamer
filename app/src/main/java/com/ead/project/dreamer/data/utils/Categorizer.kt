...
object Categorizer {
    ...
    private fun getListFromRawGenres(string: String) : List<String> = string
        .replace("[","")
        .replace("]","")
        .split(",")
    ...
    fun configProfiles(animeProfileList : List<AnimeProfile>) : List<String> {
        ...
        return listOf(
            x.getCatch(0),x.getCatch(1),
            x.getCatch(2),x.getCatch(3),
            x.getCatch(4), y.getCatch(0),
            y.getCatch(1),y.getCatch(2),
            z.getCatch(0))
    }
